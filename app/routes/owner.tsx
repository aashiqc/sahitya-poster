  ...
  return (
    <div className="relative min-h-dvh bg-[#F7F4EC] text-stone-900">
      ...
      {/* Delete-tenant modal — rendered at the page root so it sits
          ...
          or Escape closes it (the action's success redirect would
          remove the param anyway). */}
      {deleteOpen && deleteTenant && (
        ...
        >
          <div
            className="relative w-full max-w-md overflow-hidden rounded-2xl border border-red-200 bg-white shadow-[0_24px_60px_-20px_rgba(0,0,0,0.4)]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-baseline justify-between gap-2 border-b border-red-100 bg-red-50/60 px-5 py-3">
              <h2
                id="delete-tenant-title"
                className="font-[Fraunces,serif] text-base tracking-tight text-red-700"
              >
                Delete tenant
              </h2>
              ...
              >
                ...
                <label className="block space-y-1">
                  ...
                  <input
                    ...
                    required
                    autoComplete="off"
                    // Native autofocus so the owner can start typing
                    // the subdomain immediately. The empty-deps effect
                    // pattern is overkill for a single mount.
                    autoFocus
                    className={`${field} font-mono`}
