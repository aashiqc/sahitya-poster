...
export async function loadTenantEvent(
  ...
  const { data, error } = await supabase
    .from("events")
    .select(
      "id, slug, name, name_ml, status, starts_on, ends_on, organization_id, standings_template, final_poster_url, result_template, poster_lang, poster_name, poster_date, poster_time, poster_place, organizations!inner(name, slug, subdomain)",
    )
